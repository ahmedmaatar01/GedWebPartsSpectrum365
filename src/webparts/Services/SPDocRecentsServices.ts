...
export class SPOperations {
    ...
    public async GetRecentFiles(context: WebPartContext, listTitle: string, timeInterval: string): Promise<SPListItem[]> {
        const now = new Date();
        let pastDate: Date;
        ...
        const restApiUrl = `${context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('${listTitle}')/items?$filter=Modified ge '${pastDateString}'&$orderby=Modified desc&$select=Title,FileLeafRef,Created,Author/Title,Editor/Title,FileSystemObjectType&$expand=Author,Editor`;
